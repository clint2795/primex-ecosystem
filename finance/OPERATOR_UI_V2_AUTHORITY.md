...
## Proper app / program target

Operator UI v2 is not a browser-only end state. It must be designed so the same product can become a proper commercial application across:
- Windows/macOS desktop app
- iPhone/iPad and Android app
- browser/web access where useful

The long-term product should share one business/domain layer underneath and support reusable UI components, company onboarding/configuration, roles/permissions, secure sign-in, cloud sync, resilient local/offline behaviour where sensible, notifications, version/update handling, backup/audit, licensing/subscriptions, and installer/App Store/Play Store distribution paths later.

This does **not** mean implementing every packaging/runtime concern now. It means avoiding browser-only assumptions in the v2 architecture and keeping business logic separated from presentation so the product can later be packaged for desktop, mobile and web without rewriting the Finance engine.

## Content-economy gate

Every visible element must earn its place.

Before any v2 screen is accepted, apply these checks:
1. **Duplicate check** — do not repeat context the user already has.
2. **Purpose check** — each element must help the operator understand, decide, or act.
3. **Level check** — global app controls belong in the app shell; page/job controls belong in the job.
4. **Priority check** — the important thing must be obvious within about two seconds.
5. **Removal pass** — delete anything that can disappear with zero loss of meaning.
6. **Mobile check** — vertical space is expensive; avoid unnecessary labels, headings and helper copy.
7. **Operator test** — do not add text just because it looks like normal UI copy.

Do not ship duplicate context, designer commentary, redundant helper text, repeated section names, or controls that are visually attached to the wrong level of the interface.

Operational names must be relationship-first, not decorative headline-first: a person/company name must be visibly attached to its role (Customer, Supplier, Account, etc.) rather than floating as an unexplained heading.
